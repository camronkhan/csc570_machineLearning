...
```{r}
preprocessor = preProcess(rawData[2:15], c("center", "scale"))
...
```{r}
for (feature in names(preprocessedData)) {
  plot <- ggplot(preprocessedData, aes(x = preprocessedData[[feature]]), binwidth = 2) + 
            geom_histogram(aes(y = ..density..), fill = 'red', alpha = 0.5, bins = 40) + 
            geom_density(colour = 'blue') + xlab(expression(bold('Values (Standardized)'))) + 
            ylab(expression(bold('Density'))) + ggtitle(feature)
  print(plot)
}
```
