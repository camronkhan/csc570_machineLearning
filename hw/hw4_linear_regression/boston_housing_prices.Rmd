...
```{r}
ggplot(rawData, aes(x = lapply(rawData$black, function(y) y^3)), binwidth = 2) + 
            geom_histogram(aes(y = ..density..), fill = 'red', alpha = 0.5, bins = 40) + 
...
```{r}
trans <- lapply(rawData$black, function(x) x^3)
print(head(trans))
print(head(rawData$black))
```
